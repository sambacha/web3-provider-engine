...
class BlockCacheStrategy {
    ...
    async get(payload, requestedBlockNumber) {
        ...
        const identifier = cache_utils_1.cacheIdentifierForPayload(payload, true);
    ...
    canCacheRequest(payload) {
        ...
        if (!cache_utils_1.canCache(payload)) {
            return false;
        }
        // check blockTag
        const blockTag = cache_utils_1.blockTagForPayload(payload);
        if (blockTag === 'pending') {
            return false;
        }
        // can be cached
...
function createBlockCacheMiddleware({ blockTracker } = {}) {
    ...
    return json_rpc_engine_1.createAsyncMiddleware(async (req, res, next) => {
        ...
        const type = cache_utils_1.cacheTypeForPayload(req);
        ...
        let blockTag = cache_utils_1.blockTagForPayload(req);
        if (!blockTag) {
            blockTag = 'latest';
        }
